...
for vmid in $(jq -r '.data[] | select(.name | contains("talos")) | .vmid' <<< "$response"); do
    if [[ "$vmid" -gt 199 ]]; then
        node=$(jq -r '.data[] | select(.vmid == '$vmid') | .node' <<< "$response")
        echo "Stopping VM $vmid on node $node"
        shutdown_response=$(curl -s -H "Authorization: PVEAPIToken=${PROXMOX_ID}!${PROXMOX_TOKEN}=${PROXMOX_SECRET}" \
            https://${PROXMOX_URL}/api2/json/nodes/${node}/qemu/${vmid}/status/stop -X POST \
            --data-raw 'overrule-shutdown=1')
    fi
done
