...
schematic_id=$(curl -sL https://factory.talos.dev/schematics --data-raw '{"customization":{"systemExtensions":{"officialExtensions":["siderolabs/i915-ucode","siderolabs/intel-ucode","siderolabs/qemu-guest-agent"]}}}' | jq -r '.id')
talos_version=$(curl -sL https://api.github.com/repos/talos-systems/talos/releases/latest | jq -r '.tag_name')
...
jq -n --arg intel_ucode_version "$intel_ucode_version"\
 ...
 --arg schematic_id "$schematic_id" \
  '{"intel_ucode_version":$intel_ucode_version, "amd_ucode_version":$amd_ucode_version, "qemu_ga_version":$qemu_ga_version, "imager_version":$imager_version, "talos_version":$talos_version, "schematic_id":$schematic_id}'
